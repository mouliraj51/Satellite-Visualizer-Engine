...
class Satellite:
    ...
    def compute(self):
        ...
        self.label = pyglet.text.Label(
            self.e.name, y=15, anchor_x="center", color=(255, 255, 255, 200))
    ...
    def init_line(self):
        ...
        for x in self.lines:
            self.vline_list.append(
                pyglet.graphics.vertex_list(int(len(x)/3), ("v3f", x)))
...
def init():
    ...
    for lat in range(-90, 90, step):
        ...
        vlist = pyglet.graphics.vertex_list(
            int(len(verts)/3), ('v3f', verts), ('t2f', texc), ('n3f', noramls))
        vlists.append(vlist)
...
def satellite_shape():
    ...
    noramls = verts
    vlist = pyglet.graphics.vertex_list(
        int(len(verts)/3), ('v3f', verts), ('n3f', noramls))
    vlists.append(vlist)
    return vlists
