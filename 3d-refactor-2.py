...
APIkey = "AIzaSyAcWuvXEeFJnHNIWwgvmBqto2oXHuv0JPg"
...
class Satel:
    def __init__(self, name, line_1, line_2):
        ...
        self.label = pyglet.text.Label(
            self.ep.name, y=15, anchor_x="center", color=(255, 255, 255, 200))
        self.compute()
    ...
    def compute(self):
        ...
        self.x = -cos(radians(self.lat)) * \
            cos(radians(self.long)) * self.radius
    ...
    def draw_line(self):
        ...
        for x in range(-rang, rang):
            ...
            verts += [x, y, z]
        self.line_vlist = pyglet.graphics.vertex_list(
            int(len(verts)/3), ("v3f", verts))
        glLoadIdentity()
        glTranslatef(0, 0, -zoom+6.37815)    # axis rotate by earth's surface
        glRotatef(gyration[1][0], 1, 0, 0)  # x-axis rotation
        glRotatef(gyration[1][1], 0, 0, 1)  # z-axis rotation
...
def init():
    ...
    for lat in range(-90, 90, step):
        ...
        for lon in range(-180, 181, step):
            ...
            texc += [s1, t1, s2, t2]
        earth_vlists.append(pyglet.graphics.vertex_list(
            int(len(verts)/3), ('v3f', verts), ('t2f', texc), ('n3f', verts)))

    global satel_vlist
    r = 0.08
    verts = [-r, r, r, -r, -r, r, r, r, r, r, -r, r, r, -r, -r, r, r, r, r, r, -r, -r, r,
             r, -r, r, -r, -r, -r, r, -r, -r, -r, r, -r, r, r, -r, -r, r, r, -r, -r, -r, -r, -r, r, -r]
    satel_vlist = pyglet.graphics.vertex_list(
        int(len(verts)/3), ('v3f', verts), ('n3f', verts))
    ...
    if online:
        source = urllib.request.urlopen(
            "http://www.celestrak.com/NORAD/elements/" + name + ".txt")
    else:
...
def draw_info():
    ...
    title = pyglet.text.Label(
        "3D Satellite Data Visulization", color=(255, 255, 255, 200))
    tname = pyglet.text.Label("AugSense", color=(255, 255, 255, 100))
    ctime = pyglet.text.Label("UTC Time: " + datetime.datetime.utcnow().strftime(
        "%Y-%m-%d %H:%M:%S"), color=(255, 255, 255, 200))
    csate = pyglet.text.Label(
        "Satellites Category: " + resource[num][0], color=(255, 255, 255, 200))
    cross = pyglet.graphics.vertex_list(
        4, ('v3f', [1, 0, 0, -1, 0, 0, 0, 1, 0, 0, -1, 0]))
    addre = pyglet.text.Label(geo, anchor_x="center",
...
def info_address(lat, lng):
    ...
    if geo == "":
        data = json.dumps([])
        url = "https://geocode.maps.co/reverse?lat=" + \
            str(lat) + "&lon=" + str(lng)
...
def update_contorl():
    ...
    gyration[0][1] = contorl_opp(
        [keys[key.LEFT], keys[key.RIGHT]], gyration[0][1], 0.5)
    ...
    # Reset
    if keys[key.N]:
        gyration, zoom = [[43.000809, -78.78897], [0, 0]], 100  # Back to UB
    if keys[key.Q]:
        gyration[1][0] = 0  # Reset x-axis rotation
    if keys[key.E]:
        gyration[1][1] = 0  # Reset z-axis rotation
    ...
    if keys[key.E]:
        gyration, zoom = [
            [random.uniform(-90, 90), random.uniform(-180, 180)], [0, 0]], 100
        geo = ""
    if keys[key.R]:
...
def on_mouse_drag(x, y, dx, dy, buttons, modifiers):
    ...
    # Special Configuration to Prevent QUIVER
    if gyration[0][0] - d > 88:
        gyration[0][0] = 88
    elif gyration[0][0] - d < -88:
        gyration[0][0] = -88
    else:
        gyration[0][0] -= d
...
# webview.create_window("My Webview Window",
#                       url="map.html")
# webview.start()
