...
# Input Data type: TLE(Two-line element set)
...
resource = [["GPS Operational", "gps-ops&FORMAT=tle"],
            ["Glonass Operational", "glo-ops&FORMAT=tle"],
            ["Galileo Satellites", "galileo&FORMAT=tle"],
            ["Beidou",	"beidou&FORMAT=tle"],
            ["(NNSS)", "nnss&FORMAT=tle"],
            [" (WAAS/EGNOS/MSAS)", 	"sbas&FORMAT=tle"]]
...
class Background:
    def __init__(self, x, y, xoffset, yoffset, texturefile):
        ...
        self.vlist = pyglet.graphics.vertex_list(
            4, ('v2f', [xoffset, yoffset, xoffset+x, yoffset, xoffset, yoffset+y, xoffset+x, yoffset+y]), ('t2f', [0, 0, 1, 0, 0, 1, 1, 1]))
...
class Satellite:
    ...
    def compute(self):
        ...
        self.label = pyglet.text.Label(
            self.e.name, x=7, y=0, anchor_y="center", color=(255, 255, 255, 255))
    ...
    def init_line(self):
        ...
        for x in range(-total, total):
            ...
            y = (math.degrees(float(self.e.sublat))
                 * 128/45) + 256 + self.yoffset
        ...
        self.lines.append(current_line)
        for x in self.lines:
            self.vline_list.append(
                pyglet.graphics.vertex_list(int(len(x)/2), ("v2f", x)))
...
def init():
    ...
    text_current_set = pyglet.text.Label(
        "Satellites Category on Screen: " + resource[category_num][0], x=15, y=42, anchor_y="center", color=(255, 255, 255, 200))
    text_current_time = pyglet.text.Label("Current UTC Time: " + datetime.datetime.utcnow(
    ).strftime("%Y-%m-%d %H:%M:%S"), x=15, y=22, anchor_y="center", color=(255, 255, 255, 200))
    text_infos = pyglet.text.Label("Click a satellite to display its orbit, Press H to show/hide all orbits on-screen.",
                                   x=460, y=50, anchor_y="center", color=(255, 255, 255, 200))
    text_infos_2 = pyglet.text.Label("Press UP/DOWN to change satellite category.",
                                     x=460, y=32, anchor_y="center", color=(255, 255, 255, 200))
    text_infos_3 = pyglet.text.Label("Press LEFT/RIGHT to adjust orbit interval for line drawing.",
                                     x=460, y=14, anchor_y="center", color=(255, 255, 255, 200))
...
def open_new_file(num):
    ...
    name = resource[num][1]
    url = "http://www.celestrak.com/NORAD/elements/gp.php?GROUP=" + name
